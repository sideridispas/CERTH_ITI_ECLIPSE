...
static const char remote_device_name[] = "Random Nameeee";
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            //ESP_LOGI(GATTC_TAG, "searched Device Name Len %d, RSSI %d", adv_name_len, scan_result->scan_rst.rssi);
            ...
            if (adv_name != NULL) {
            	...
            	ESP_LOGI(GATTC_TAG, "--not in range for unlock-- \n");

            	if(abs(prox) <=60){
            		ESP_LOGI(GATTC_TAG, "==== DEVICE IN RANGE ====");
            		esp_log_buffer_char(GATTC_TAG, adv_name, adv_name_len);
            		ESP_LOGI(GATTC_TAG, "RSSI: %d ", prox);
            		...
            		if (connect == false) {
            		    ...
            		    ESP_LOGI(GATTC_TAG, "========USER RIGHTS CHECK PASS (ROOM ID)");
            		    ESP_LOGI(GATTC_TAG, "============UNLOCK DOOR");
            		}
            	...
            	/*if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ...
                    if(abs(prox) <=60){
                    	if (connect == false) {
                    		connect = true;
                    	    //ESP_LOGI(GATTC_TAG, "connect to the remote device.");
                    	    ESP_LOGI(GATTC_TAG, "====PROXIMITY CHECK PASS (RSSI < 60)");
                    	    esp_ble_gap_stop_scanning();
                    	    esp_ble_gattc_open(gl_profile_tab[PROFILE_A_APP_ID].gattc_if, scan_result->scan_rst.bda, true);
